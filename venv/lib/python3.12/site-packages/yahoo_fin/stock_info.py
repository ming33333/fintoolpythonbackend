...
def get_quote_table(ticker , dict_result = True, headers = {'User-agent': 'Mozilla/5.0'}): 
    ...
    print("tables", requests.get(site, headers=headers).text)
...
def _parse_json(url, headers = {'User-agent': 'Mozilla/5.0'}):
    ...
    json_str = html.split('root.App.main =')[0].split(
        '(this)')[0].split(';\n}')[0].strip()
    ...
    try:
        ...
        try:
            start = html.find('data-ttl="1">')
            while start != -1:
                end = html.find('</script>', start)
                if end == -1:
                    break
                script_content = html[start+len('data-ttl="1">'):end]
                if 'quoteSummary' in script_content:
                    json_str = script_content
                    break
                start = html.find('data-ttl="1">', end)
            data = json.loads(json.loads(json_str)["body"])["quoteSummary"]['result'][0]
        except:
            return '{}'
    if data:
        # return data
